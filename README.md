...
#### Основные элементы целевой конфигурации n8n‑воркфлоу
- Базовые метаданные: содержат как минимум `version` (версия формата) и человекочитаемое `name`. Это позволяет контроллеру и оркестратору однозначно идентифицировать артефакт и его совместимость.
- Структура из нод (`nodes`): каждая нода соответствует шагу автомations и описывает `id`, `type`, `position` и `parameters`. Именно эти сущности создаёт/обновляет Developer по плану Controller.
- Связи (`links`/`edges`): соединения между выходами и входами нод фиксируются явно (например, поля `from`, `to`, `output`, `input`). Это необходимо для детерминированного сравнения желаемого и фактического графа и для генерации плана действий.
- Параметры и соединения: при генерации плана Controller/Developer должны учитывать не только наличие нод, но и значения их `parameters`, а также корректность `links`. Без этого QA не сможет подтвердить правильность конфигурации.
- Операционные метаданные: рекомендуется включать поля для операционного управления, например `metadata.tags` и `metadata.active`. Этими атрибутами управляет Manager после основного цикла (переименование, включение/отключение, теги).
- Требования к качеству: целевая модель должна содержать достаточный контекст для проверок QA (ожидаемые заголовки узлов, ключевые параметры/значения, наличие необходимых связей). Это делает автоматические проверки воспроизводимыми и объективными.
